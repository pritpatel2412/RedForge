function parseCSP(value) {
    ...
    if (value.includes("'unsafe-eval'"))
        issues.push("'unsafe-eval' allows eval() execution (XSS risk)");
    if (/\bscript-src\b[^;]*\*/.test(value) || /\bdefault-src\b[^;]*\*/.test(value))
        issues.push("Wildcard (*) source allows scripts from any domain");
    if (!value.includes("default-src"))
        issues.push("Missing default-src directive (fallback not set)");
    if (!value.includes("frame-ancestors"))
        issues.push("Missing frame-ancestors directive (clickjacking risk)");
    return issues;
}
export async function runHeadersModule(ctx) {
    const findings = [];
    ...
    const h = (name) => resHeaders[name] || resHeaders[name.toLowerCase()] || "";
    ...
    if (reachable && (targetUrl.includes("/auth") || targetUrl.includes("/login") ||
