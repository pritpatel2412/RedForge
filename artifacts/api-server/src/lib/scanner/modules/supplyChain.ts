...
function extractLibFromUrl(url: string): { name: string; version: string } | null {
  // jsdelivr: /npm/library@version/file.js
  const jsdelivrMatch = url.match(/jsdelivr\.net\/npm\/([^@/]+)@([^/]+)/i);
  if (jsdelivrMatch) return { name: jsdelivrMatch[1], version: jsdelivrMatch[2] };
  ...
  if (unpkgMatch) return { name: unpkgMatch[1], version: unpkgMatch[2] };

  return null;
}

async function checkOSVVulnerabilities(packageName: string, version: string): Promise<{ vulnId: string; summary: string; severity: string }[]> {
  try {
    ...
    return vulns.slice(0, 3).map((v: any) => ({
      vulnId: v.id || "UNKNOWN",
...
export async function runSupplyChainModule(ctx: ScanContext): Promise<FindingInput[]> {
  ...
  const cdnScriptsWithoutSRI = externalScripts.filter(s =>
    !s.hasSRI && !s.isHttp &&
  ...
  if (cdnLibsToCheck.length > 0) {
    ...
    for (const lib of cdnLibsToCheck) {
      if (!lib.libName || !lib.libVersion) continue;
