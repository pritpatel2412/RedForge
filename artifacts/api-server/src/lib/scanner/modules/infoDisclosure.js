...
export async function runInfoDisclosureModule(ctx) {
    const findings = [];
    ...
    if (reachable && bodyText) {
        ...
        for (const scriptUrl of scriptUrls.slice(0, 6)) {
            if (scriptUrl.includes("cdn") || scriptUrl.includes("unpkg") || scriptUrl.includes("jsdelivr"))
                continue;
    ...
    try {
        ...
        if (result && result.status === 200 && result.body.length > 10 && !result.body.toLowerCase().includes("<!doctype html")) {
            const disallowedLines = result.body.split("\n")
                ...
                .filter(p => p && p !== "/");
            const sensitivePaths = disallowedLines.filter(p => SENSITIVE_ROBOTS_PATHS.some(sp => p.toLowerCase().includes(sp)));
    ...
    const sensitivePaths = [
        { path: "/.env", label: ".env secrets file", severity: "CRITICAL", cvss: "9.8" },
        { path: "/.git/HEAD", label: "Git repository exposed", severity: "CRITICAL", cvss: "9.1" },
        { path: "/config.json", label: "Config JSON exposed", severity: "HIGH", cvss: "7.5" },
        { path: "/api/health", label: "Health endpoint", severity: "LOW", cvss: "3.7" },
        { path: "/api/version", label: "Version endpoint", severity: "LOW", cvss: "3.7" },
        { path: "/swagger.json", label: "Swagger/OpenAPI docs", severity: "MEDIUM", cvss: "5.3" },
        { path: "/openapi.json", label: "OpenAPI spec exposed", severity: "MEDIUM", cvss: "5.3" },
        { path: "/graphql", label: "GraphQL endpoint", severity: "MEDIUM", cvss: "5.3" },
        { path: "/admin", label: "Admin panel", severity: "HIGH", cvss: "7.5" },
        { path: "/wp-admin", label: "WordPress admin", severity: "HIGH", cvss: "7.5" },
        { path: "/phpmyadmin", label: "phpMyAdmin exposed", severity: "CRITICAL", cvss: "9.8" },
        { path: "/.well-known/security.txt", label: "security.txt", severity: "LOW", cvss: "0.0" },
    ];
    ...
    for (const sp of sensitivePaths) {
        let probeUrl;
        try {
            probeUrl = new URL(sp.path, targetUrl).toString();
        }
        catch {
            continue;
        }
