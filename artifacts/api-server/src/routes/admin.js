...
import { db, usersTable, workspacesTable, couponsTable, userActivityLogsTable, emailLogsTable } from "@workspace/db";
import { eq, desc, count, and, gte, lte, sql, isNotNull, inArray } from "drizzle-orm";
    ...
    try {
        ...
        if (userIds.length > 0) {
            ...
            for (const a of activities) {
                if (a.userId)
                    lastActivities[a.userId] = a.lastSeen;
            }
        }
        res.json({
