...
async function addLog(scanId, level, message) {
    await db.insert(scanLogsTable).values({ scanId, level, message });
    await sleep(200 + Math.random() * 300);
}
export async function runRealScan(scanId, rawTargetUrl, scanMode = "PASSIVE") {
    ...
    try {
        ...
        const scanCtx = {
            ...
            resHeaders,
            bodyText,
            addLog: (level, message) => addLog(scanId, level, message),
        };
        ...
        if (scanMode === "ACTIVE") {
            ...
            if (!sqliFound)
                await addLog(scanId, "DEBUG", "No SQL error patterns detected ✓");
            ...
            for (const ep of authEndpoints) {
                const results = await Promise.all(Array.from({ length: 6 }, (_, i) => safeFetch(ep, {
                    method: "POST",
        ...
        if (nimKey) {
            const runAiAnalysis = async (model) => {
...
Only include findings you are confident about. If none, return [].`;
        ...
        if (project) {
            ...
            if (webhookUrl) {
                ...
                await sendScanComplete({ id: scanId, findingsCount: findings.length, criticalCount, highCount }, project.name, APP_URL, webhookUrl).catch(() => { });
            }
        }
    }
    catch (err) {
