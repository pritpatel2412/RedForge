...
export function buildWelcomeEmail(name, trialEndsAt) {
...
export function buildTrialExpiringEmail(name, daysLeft, trialEndsAt) {
...
export function buildPlanChangedEmail(name, newPlan, changedBy = "admin") {
    return emailLayout(`
    <div class="card">
...
export function buildCouponAppliedEmail(name, couponCode, benefit) {
    return emailLayout(`
    <div class="card">
...
async function logEmail(to, subject, template, status, error, metadata) {
    try {
...
export async function sendWelcomeEmail(user, trialEndsAt) {
    await sendEmail({
        to: user.email,
...
export async function sendTrialExpiringEmail(user, daysLeft, trialEndsAt) {
    await sendEmail({
        to: user.email,
        subject: daysLeft <= 1
...
export async function sendPlanChangedEmail(user, newPlan, changedBy = "admin") {
    await sendEmail({
        to: user.email,
...
export async function sendCouponAppliedEmail(user, couponCode, benefit) {
    await sendEmail({
        to: user.email,
