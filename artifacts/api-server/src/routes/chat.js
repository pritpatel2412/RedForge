        ...
        : highFindings.length > 3 ? "🟠 HIGH RISK"
            : mediumFindings.length > 5 ? "🟡 MEDIUM RISK"
                : allFindings.length > 0 ? "🟢 LOW RISK"
                    : "⚪ NOT ASSESSED";
    const findingsContext = allFindings.slice(0, 40).map((f) => `• [${f.severity}] ${f.title}\n  Endpoint: ${f.endpoint}\n  CVSS: ${f.cvss || "N/A"} | OWASP: ${f.owasp || "N/A"} | CWE: ${f.cwe || "N/A"}\n  Status: ${f.status} | Project: ${f.projectName}\n  Description: ${(f.description || "").slice(0, 200)}`).join("\n\n");
    const projectsContext = projects.map((p) => `• ${p.name} → ${p.targetUrl} (${p.targetType})`).join("\n");
    const scansContext = recentScans.slice(0, 5).map((s) => `• Scan ${s.id.slice(0, 8)} | Status: ${s.status} | Findings: ${s.findingsCount || 0} (${s.criticalCount || 0} critical)`).join("\n");
...
router.post("/conversations/:id/messages", requireAuth, async (req, res) => {
    ...
    if (Array.isArray(msgs) && msgs.length > 0) {
        await db.insert(chatMessagesTable).values(msgs.map((m) => ({
            conversationId: id,
            role: m.role,
            content: m.content,
...
router.post("/", requireAuth, async (req, res) => {
    ...
    const runStream = async (model, isFallback = false) => {
